...
| D | `D_ut` | DMD 2.109.0 (compiled with `-unittest`) |
...
make build-base-image       # produces sandbox:base

# Build the server image (adds the app + Python deps on top of sandbox:base)
make build-server-image     # produces sandbox:server
